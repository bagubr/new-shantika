        ...
        <div class="col">
            <div class="card card-primary">
                ...
                <div class="card-body" style="display: block;">
                    ...
                    <form action="{{route('route_setting.store')}}" method="POST">
                        @csrf
                        <div class="form-row">
                            <div class="col">
                                <div class="form-group">
                                    <input type="hidden" name="fleet_route_id" id="" value="{{ $fleet_route->id }}">
                                    <label>Rute</label><span class="text-danger">*</span>
                                    <input type="text" name="fleet_route_name" id="" value="{{ $fleet_route->route->name }}" disabled class="form-control">
                                </div>
                                <div class="form-group">
                        ...
                        <a href="{{route('route_setting.index', ['area_id' => $fleet_route->route->checkpoints[0]?->agency?->city?->area?->id])}}" class="btn btn-secondary">Batal</a>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @foreach ($route_settings as $route_setting)
                                        <tr>
                                            <td>{{$route_setting->agency->name}}</td>
                                            <td>{{ date('Y-m-d', strtotime($route_setting->start_at)) }}</td>
                                            <td>{{ date('Y-m-d', strtotime($route_setting->end_at)) }}</td>
                                            <td>    
                                                <form action="{{route('route_setting.destroy', $route_setting->id)}}" class="d-inline"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
