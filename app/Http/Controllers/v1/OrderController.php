...
class OrderController extends Controller
{
    public function calculateDiscount(ApiCalculateDiscountRequest $request) {
        ...
        $price_food = ($request->is_food) ? ($fleet_route->fleet_detail?->fleet?->fleetclass?->price_food) * $request->seat_count : 5000 * $request->seat_count;
        ...
        $data = [
            'total_food'=> ($request->is_food) ? ($fleet_route->fleet_detail?->fleet?->fleetclass?->price_food) * $request->seat_count : 0,
