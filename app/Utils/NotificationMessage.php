...
class NotificationMessage {
    ...
    public static function newTicketOrder($order) {
        return [
            "Pembelian Tiket",
            "Pemesan baru pada armada ".$order->fleet_route?->fleet_detail?->fleet?->name." di kursi ".implode(",", $order->order_detail?->chair->pluck('name'))." pada tanggal ".date('d-m-Y', strtotime($order->reserve_at))
        ];
    }
