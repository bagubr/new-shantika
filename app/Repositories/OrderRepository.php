...
class OrderRepository
{
    ...
    public static function getBoughtRouteByAgencyByDate($token, $date)
    {
        ...
        $order = Order::where(function($query) use ($user) {
            ...
            ->select()
            ->addSelect(DB::raw("(select sum(total_deposit) from order_price_distributions opd left join orders o on o.id = opd.order_id left join fleet_routes on o.fleet_route_id = fleet_routes.id left join fleet_detail on fleet_detail.id = fleet_routes.fleet_detail_id where o.reserve_at::text ilike '%$date%' and o.departure_agency_id = $agency_id) as total_deposit_fleet_route"))
            ->get();
